<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="门店账号" prop="mendianzhanghao">
          <el-select :disabled="ro.mendianzhanghao" @change="mendianzhanghaoChange" v-model="ruleForm.mendianzhanghao" placeholder="请选择门店账号">
            <el-option
                v-for="(item,index) in mendianzhanghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.mendianzhanghao" label="门店账号" prop="mendianzhanghao">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="门店姓名" prop="mendianxingming">
          <el-input v-model="ruleForm.mendianxingming" 
              placeholder="门店姓名" clearable  :readonly="ro.mendianxingming"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="门店姓名" prop="mendianxingming">
              <el-input v-model="ruleForm.mendianxingming" 
                placeholder="门店姓名" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(this.$storage.get("role")!="管理员") {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/mendian/mendianzhanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.mendianzhanghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    mendianzhanghaoChange () {
      this.$http({
        url: `follow/mendian/mendianzhanghao?columnValue=`+ this.ruleForm.mendianzhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.mendianxingming){
                this.ruleForm.mendianxingming = data.data.mendianxingming
              }
        } else {
          this.$message.error(data.msg);
        }
